...
export function evaluateEnigmaAt(candles: Candle[], i: number, ctx: EnigmaContext = {}): StrategySignal | null {
  ...
  if (p >= e.bullReversalMin && ctxPressure <= e.bearContextMax) {
    ...
    if (e.useHtf) {
      if (!htf) return neutral('Setup haussier rejete : pas de donnee HTF cloturee disponible.');
      if (htf.pressure < e.htfMinPressure) {
        return neutral(`Setup haussier rejete : pression HTF ${htf.pressure.toFixed(2)} < ${e.htfMinPressure}.`);
      }
      if (htf.bodyAtr < e.htfMinBodyAtr) {
        return neutral(`Setup haussier rejete : corps HTF ${htf.bodyAtr.toFixed(2)} x ATR < ${e.htfMinBodyAtr}.`);
      }
    }
  ...
  if (p <= e.bearReversalMax && ctxPressure >= e.bullContextMin) {
    ...
    const htf = htfPressureAt(ctx.htf, bar.openTime);
    if (e.useHtf) {
      if (!htf) return neutral('Setup baissier rejete : pas de donnee HTF cloturee disponible.');
      if (htf.pressure > 1 - e.htfMinPressure) {
        return neutral(`Setup baissier rejete : pression HTF ${htf.pressure.toFixed(2)} > ${(1 - e.htfMinPressure).toFixed(2)}.`);
      }
      if (htf.bodyAtr < e.htfMinBodyAtr) {
        return neutral(`Setup baissier rejete : corps HTF ${htf.bodyAtr.toFixed(2)} x ATR < ${e.htfMinBodyAtr}.`);
      }
    }
    if (e.requireConfirmation) {
